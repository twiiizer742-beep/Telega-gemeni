...
// 1. Фейковый сервер для Render (чтобы не было ошибки портов)
...
const SYSTEM_PROMPT = `Ты — Кирилл, эксперт. Пиши просто. 
ЗАПРЕЩЕНО: звездочки (*), решетки (#), линии (---), римские цифры. 
...
bot.on('text', async (ctx) => {
  ...
  try {
    ...
    // ПРИНУДИТЕЛЬНАЯ ОЧИСТКА ОТ МУСОРА
    ...
    if (aiText.length > 4000) {
      await ctx.reply(aiText.substring(0, 3900) + '... (текст обрезан)', getMainMenu());
    } else {
      await ctx.reply(aiText, getMainMenu());
    }
  } catch (e) {
    console.error('Error:', e.message);
    await ctx.reply('Ошибка. Попробуй позже.', getMainMenu());
  }
});
